  ...
  Scenario: net with static vlans
    Given a set of 2 switches
     ...
     When we connect switch s1 to switch s2
      ...
      And we assign host h1 on switch s1 to VLAN 888
      And we assign host h2 on switch s2 to VLAN 10
...
#     Then the http traffic from host h1 to host h2 takes the route across switch s5