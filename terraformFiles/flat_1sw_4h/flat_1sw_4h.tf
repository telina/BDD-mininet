...
#FLOATING IP
resource "openstack_compute_floatingip_v2" "floatip_0" {
  pool = "net04_ext"
}
...
#VM switch
resource "openstack_compute_instance_v2" "default_Instance_0" {
  name = "switch"
...
#  key_pair = "${openstack_compute_keypair_v2.bpf.name}"
  floating_ip = "${openstack_compute_floatingip_v2.floatip_0.address}"
  ...
  provisioner "remote-exec" {
      ...
      inline = [
          "sudo sed -i 's/ localhost/ localhost switch/' /etc/hosts",
         ...
         # "sudo apt-get -y upgrade",
          ...
          "sudo ovs-vsctl add-port myBridge eth4",
          ...
          "sudo ovs-ofctl add-flow myBridge 'in_port=4,priority=10,actions=all'",
